package com.mitnick.tannotour.smartutlis.gaea.clock;
...
public class MiClockView extends View {
    ...
    /* 测量磁场强度文本宽高的矩形 */
    private Rect mMagneticTextRect;
    /* 测量磁场强度单位文本宽高的矩形 */
    private Rect mMagneticUnitRect;
    /* 小时圆圈画笔 */
    private Paint mCirclePaint;
    /* 磁场强度画笔 */
    private Paint magneticPaint;
    /* 小时圆圈线条宽度 */
    ...
    /* 磁场强度数值 */
    private String magneticText = "0";
    /* 磁场强度单位 */
    private String magneticUnit = "uT";
    /* 磁场强度数字大小 */
    private int magneticTextSize = 120;
    /* 颜色渐变插值器 */
    ...
    public MiClockView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        magneticPaint = new Paint();
        magneticPaint.setColor(mDarkColor);

        mMagneticTextRect = new Rect();
        mMagneticUnitRect = new Rect();
    }
...
//        mHourDegree = hour / 12 * 360;
        //根据磁场强度计算秒针的位置 start
        float magneticTextF = Float.valueOf(magneticText);
        if (magneticTextF > 80f){
            mSecondDegree = 360f;
        }else{
            mSecondDegree = magneticTextF / 80f * 360f;
        }
        if(magneticTextF <= 70f){
            magneticPaint.setColor(mDarkColor);
            //设置渐变遮罩层 start
            mSweepGradient = new SweepGradient(width / 2, height / 2,
                    new int[]{mDarkColor, mLightColor}, new float[]{0.75f, 1});
            //设置渐变遮罩蹭 stop
        }else{
            Integer color = (Integer) argbEvaluator.evaluate(
                    (mSecondDegree - 70) / 360f,
                    Color.rgb(255, 255, 255),
                    Color.rgb(229, 28, 35)
            );//颜色插值器
            magneticPaint.setColor(color);
            //设置渐变遮罩层 start
            mSweepGradient = new SweepGradient(width / 2, height / 2,
                    new int[]{mDarkColor, color}, new float[]{0.75f, 1});
            //设置渐变遮罩蹭 stop
        }
        //根据磁场强度计算秒针的位置 stop
    }
    ...
    private void drawTimeText() {
        ...
        int textLargeWidth = mTextRect.width();//一位数字的宽
        mCanvas.drawText("0", getWidth() / 2 - textLargeWidth / 2, mPaddingTop + mTextRect.height(), mTextPaint);
        timeText = "20";
        mTextPaint.getTextBounds(timeText, 0, timeText.length(), mTextRect);
        int textSmallWidth = mTextRect.width();//两位数字的宽
        mCanvas.drawText("20", getWidth() - mPaddingRight - mTextRect.height() / 2 - textSmallWidth / 2,
                getHeight() / 2 + mTextRect.height() / 2, mTextPaint);
        mCanvas.drawText("40", getWidth() / 2 - textSmallWidth / 2, getHeight() - mPaddingBottom, mTextPaint);
        mCanvas.drawText("60", mPaddingLeft + mTextRect.height() / 2 - textSmallWidth / 2,
                getHeight() / 2 + mTextRect.height() / 2, mTextPaint);
     ...
     * 画磁场强度
     */
    private void drawMgneticText(){
        magneticPaint.setTextSize(magneticTextSize);
        magneticPaint.getTextBounds(magneticText, 0, magneticText.length(), mMagneticTextRect);
        int magneticTextWidth = mMagneticTextRect.width();
        int magneticTextHeight = mMagneticTextRect.height();
        magneticPaint.setTextSize(magneticTextSize/2);
        magneticPaint.getTextBounds(magneticUnit, 0, magneticUnit.length(), mMagneticUnitRect);
        int magneticUnitWidth = mMagneticUnitRect.width();
        int magneticUnitHeight = mMagneticUnitRect.height();
        mCanvas.drawText(
                magneticUnit,
                getWidth()/2 - (magneticTextWidth + magneticUnitWidth)/2 + magneticTextWidth + 30,
                getHeight()/2 + magneticTextHeight/2,
                magneticPaint
        );
        magneticPaint.setTextSize(magneticTextSize);
        mCanvas.drawText(
                magneticText,
                getWidth()/2 - (magneticTextWidth + magneticUnitWidth)/2,
                getHeight()/2 + magneticTextHeight/2,
                magneticPaint
        );
    }
    ...
    public void setMagneticText(String magneticText) {
        this.magneticText = magneticText;
    }

    public void setMagneticTextSize(int magneticTextSize) {
        this.magneticTextSize = magneticTextSize;
    }
    ...
    public void setmLightColor(int mLightColor) {
        this.mLightColor = mLightColor;
    }

    public void setmDarkColor(int mDarkColor) {
